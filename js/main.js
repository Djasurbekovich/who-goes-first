...
elDistanceForm.addEventListener("submit", function (e) {
  ...
  if (distance > 0) {
    ...
    walkMinute = Math.round(
      (parseFloat(walkTime.toFixed(2), 10) - walkHour) * 60
    );

    let bikeTime = distance / bikeSpeed;
    bikeHour = parseInt(bikeTime, 10);
    bikeMinute = Math.round(
    ...
    planeMinute = Math.round(
      (parseFloat(planeTime.toFixed(2), 10) - planeHour) * 60
    );
